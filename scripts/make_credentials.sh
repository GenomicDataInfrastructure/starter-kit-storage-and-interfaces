...
for n in download finalize inbox ingest mapper orch sync verify; do
...
[default]
access_key=dummy_gdi_eu
secret_key=dummy_gdi_eu
access_token=$token
...
## create crypt4gh key
curl -L https://github.com/neicnordic/crypt4gh/releases/download/v"${C4GH_VERSION}"/crypt4gh_linux_x86_64.tar.gz | tar -xz -C /shared/ && chmod +x /shared/crypt4gh
